...
class MCPMetadataParser:
    ...
    KNOWN_CREDENTIAL_PATTERNS = {
        ...
        "oauth_token": r".*(OAUTH|ACCESS)_TOKEN$",
    ...
    @classmethod
    def parse_server_metadata(cls, server_config: Dict[str, Any]) -> List[CredentialRequirement]:
            ...
            for cred in server_config["required_credentials"]:
                required_credentials.append(
                    CredentialRequirement(
                        ...
                        name=cred["name"],
                        description=cred.get("description", f"Credential: {cred['name']}"),
    ...
    def _extract_credential_name(env_var: str) -> str:
        ...
        filtered_words = []
        for word in words:
            if word.upper() not in prefixes_to_remove and word.upper() not in suffixes_to_remove:
