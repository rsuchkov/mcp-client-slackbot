...
from ..database.models import MCPServer, User
from .mcp_metadata import MCPMetadataParser


class UserMCPServer:
    def __init__(
        self, user: User, server_config: MCPServer, credentials: Dict[str, str]
    ):
        self.user = user
    ...
    async def initialize(self) -> bool:
        try:
            ...
            env = MCPMetadataParser.build_env_with_credentials(
                self.server_config.env,  # type: ignore[arg-type]
                self.credentials,
                self.server_config.required_credentials,  # type: ignore[arg-type]
            )

            server_params = StdioServerParameters(
                command=self.server_config.command,  # type: ignore[arg-type]
                args=self.server_config.args,  # type: ignore[arg-type]
                env=env,
            )
            ...
            print(
                f"Failed to initialize server {self.server_config.name} "
                f"for user {self.user.slack_user_id}: {e}"
            )
            await self.cleanup()
            return False
    ...
    async def call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Any:
        ...
        try:
            response = await asyncio.wait_for(
                self.session.call_tool(tool_name, arguments=arguments), timeout=30.0
            )
            return response
        except Exception as e:
...
class UserServerManager:
    ...
    async def get_user_servers(self, user: User) -> List[UserMCPServer]:
        return [
            server
            for server_id, server in self.user_servers.items()
    ...
    async def cleanup_user_servers(self, user: User):
        ...
        servers_to_remove = [
            server_id
            for server_id in self.user_servers.keys()
