...
from ..database.repositories import UserRepository, CredentialRepository, ServerRepository
from ..database.session import get_session
from .mcp_metadata import MCPMetadataParser, CredentialRequirement
import json
import asyncio
...
class SlackAuthService:
    ...
    async def request_credentials(
        ...
        blocks = self._build_credential_request_blocks(server_name, missing_credentials[0], 0, len(missing_credentials))
        ...
        blocks = [
            {
                ...
                "text": {
                    "type": "mrkdwn",
                    "text": f"🔐 *Credential Setup for {server_name}*\n\nStep {current_index + 1} of {total_count}"
                }
            },
            {
                "type": "section",
                    ...
                    blocks=[
                        {
                            ...
                            "text": {
                                "type": "mrkdwn",
                                "text": f"✅ *Credentials for {flow_data['server_name']} saved successfully!*\n\nYou can now use this MCP server."
                            }
                        }
                    ]
                )
    ...
    async def _save_collected_credentials(self, flow_data: Dict[str, Any]):
        async with get_session() as session:
            ...
            user = await user_repo.get_user_by_slack_id(
                flow_data["user_slack_id"],
                self.slack_client.team_id
            )
            
            if not user:
                user_info = self.slack_client.users_info(user=flow_data["user_slack_id"])
                user = await user_repo.create_user(
                    slack_user_id=flow_data["user_slack_id"],
                    slack_team_id=self.slack_client.team_id,
    ...
    async def check_missing_credentials(
        ...
        async with get_session() as session:
            ...
            user_credentials = await cred_repo.get_user_credentials(user.id)
